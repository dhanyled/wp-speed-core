...
class AssetManagerPanel {
    ...
    public function render_panel(): void {
        ...
        // Normalize legacy format (associative array by handle) to multi-rule list format
        ...
        $existing_handles = array_column($rules, 'handle');
        $default_samples = [
        ...
        foreach ($default_samples as $sh => $stype) {
            if (!in_array($sh, $existing_handles, true)) {
                $rules[] = [
                    'handle'     => $sh,
                    'type'       => $stype,
                    'everywhere' => 0,
